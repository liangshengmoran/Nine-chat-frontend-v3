<template>
	<div class="queue">
		<div v-if="!musicStore.music_queue_list.length" class="empty">
			<icon name="choose-music-empty" scale="16" class="m_b30" />
			<span class="tips">一首都没有、还不快来点</span>
		</div>
		<div v-else class="queue-content">
			<div v-for="(item, index) in musicStore.music_queue_list" :key="index" class="music">
...
function tips() {
	ElMessage.info('正在加班加点开发ing');
}

function removeQueueMusic(musicInfo) {
	emitSocket('removeQueueMusic', musicInfo);
}
</script>
